...
tools = [
    ...
    },
    {
        ...
        "function": {
            "name": "pepper_mood",
            "description": "이 함수는 문의자가 감정을 표현하고자 할 때 호출됩니다. 감정을 나타내는 키워드가 포함된 텍스트를 입력하면, 해당 키워드에 따라 웃기, 화내기, 슬퍼하기 중 하나가 선택됩니다.",
            "parameters": {
                "type": "object",
                "properties": {
                    "감정": {
                        "type": "string",
                            "enum": ["웃기", "화내기", "슬퍼하기"],
                            "description": "질문자의 감정을 나타냅니다. 웃기, 화내기, 슬퍼하기 중 하나를 선택합니다..",
                    },
                },
...
def move(member):
    for i in location.keys():
        if i in member:
            ...
            return "-FMG-{}_{}".format(location.get(i)[0], location.get(i)[1])
    return "-FMG-None"
def dance():
    print("dance!!!!!!")
    return "-FMG-dance"
